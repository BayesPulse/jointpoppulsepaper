	...
	Priors *priors, unsigned long *seed, char *file1, double propvar[], Hyper_priors *hyper)
{
	...
	double vfepmv, vfepwv, vrem, vfem, vfew, vrew, vfemv, vfewv, vfebv, vfehv, vtime, ssq, veta[2], **pmd_var, **pmd_vch;
	Node_type *new_node;
	...
	void draw_fixed_effects(Subject_type *, Priors *, Common_parms *, unsigned long *, double, double);
	...
	void draw_eta(Subject_type *, Common_parms *, unsigned long *, double *);
	double Phi(double);
	...
	/*updated 042015 : proposal variances for subject masses and widths*/
	vfem = propvar[11];
	vfew = propvar[12];
	veta[0] = propvar[13];
	veta[1] = propvar[14];
	...
	for (i = 0; i < iter; i++) {
		...
		draw_fixed_effects(sublist, priors, parms, seed, vfem, vfew);
		...
		/*updared 2015 draw the variance component for t-distribution*/
		draw_eta(sublist, parms, seed, veta);
		...
		if (!(i%NNN)) {
			...
			subject = sublist->succ;
			printf("pct s_ma = %.2lf pct s_a = %.2lf pct A_ki = %.2lf  pct_sa= %.2lf pct_sw= %.2lf \n",
				(double)afepmv / (double)nfepmv, (double)afemv / (double)nfemv, (double)arem / (double)nrem, (double)afem / (double)nfem, (double)afew / (double)nfew);
		...
		if (!(i % 500) && i<25000 && i >0) {
			...
			adjust_acceptance((double)afew / (double)nfew, &vfew);
			adjust_acceptance((double)afem / (double)nfem, &vfem);
			adjust_acceptance((double)aetam / (double)netam, &veta[0]);
			adjust_acceptance((double)aetaw / (double)netaw, &veta[1]);
...
void draw_fixed_effects(Subject_type *sublist, Priors *priors, Common_parms *parms, unsigned long *seed, double vfem, double vfew)
{
	int j, numnode;
	double temp, theta[2], old_prior, new_prior, old_val, *tmp1, prior_ratio, like_ratio;
	double alpha;
	double prop_new, prop_old, psum_old, psum_new, pcomp,prop_ratio;
	...
	while (subject != NULL) {
		...
		tmp1[0] = afem;
		tmp1[1] = afew;
		theta[0] = rnorm(subject->theta[0], vfem, seed);
		theta[1] = rnorm(subject->theta[1], vfew, seed);
		nfem++;
		nfew++;
		...
		for (j = 0; j < 2; j++){
			...
			if (theta[j]>0) {
				/*prior */
				old_prior = (subject->theta[j] - priors->fe_mean[j])*(subject->theta[j] - priors->fe_mean[j]);
				old_prior /= priors->fe_precision[j];
				old_prior /= priors->fe_precision[j];
				new_prior = (theta[j] - priors->fe_mean[j])*(theta[j] - priors->fe_mean[j]);
				new_prior /= priors->fe_precision[j];
				new_prior /= priors->fe_precision[j];
				...
				while (subnode != NULL){
					psum_old += (subnode->theta[j] - subject->theta[j])*(subnode->theta[j] - subject->theta[j])*subnode->eta[j];
					psum_new += (subnode->theta[j] - theta[j])*(subnode->theta[j] - theta[j])*subnode->eta[j];
					stdxnew = (theta[j]) / (parms->re_precision[j] / sqrt(subnode->eta[j]));
					stdxold = (subject->theta[j]) / (parms->re_precision[j] / sqrt(subnode->eta[j]));
				...
				prop_ratio = (psum_old - psum_new)*0.5 / (parms->re_precision[j] * parms->re_precision[j]) ;
	...
	for (j = 0; j < 2; j++){
		...
		while (subject != NULL){
			...
			while (subnode != NULL){
				...
				pcomp = subnode->theta[j] - subject->theta[j];
				pcomp *= pcomp;
				psum += pcomp;
		...
		while (subnode != NULL) {
			...
			if (pRE[0] > 0.0 && pRE[1] > 0.01 && pRE[1] < 100){
				...
				for (j = 0; j < 2; j++){
					...
					prior_old = subnode->theta[j] - subject->theta[j];
...
void draw_eta(Subject_type *sublist, Common_parms *parms, unsigned long *seed, double *v)
{
	int *tmp, i, j;
		...
		while (subnode != NULL) {
			...
			for (j = 0; j<2; j++){
				...
				if (peta > 0) {
					...
					re_old = subnode->theta[j] - subject->theta[j];
					re_old *= 0.5*re_old*subnode->eta[j];
					re_new = subnode->theta[j] - subject->theta[j];
